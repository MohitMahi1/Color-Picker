...
class KMeansPixelModel:
    def __init__(self, k=5):
        ...
        self.width = None
        self.height = None
        self.original_width = None   # ← new
        self.original_height = None  # ← new
        self.scale_x = 1.0           # ← new
        self.scale_y = 1.0           # ← new
    ...
    def load_and_train(self, file):
        ...
        # Store scale ratio to map original coords → resized coords
        self.scale_x = self.width / self.original_width   # ← new
        self.scale_y = self.height / self.original_height # ← new
    ...
    def get_pixel_info(self, x, y):
        ...
        # Map original coords to resized image coords
        scaled_x = int(x * self.scale_x)  # ← new
        scaled_y = int(y * self.scale_y)  # ← new

        if not (0 <= scaled_x < self.width and 0 <= scaled_y < self.height):
            return {"error": "Out of bounds"}

        index = scaled_y * self.width + scaled_x
        cluster_id = int(self.labels[index])

        r, g, b = self.image_array[scaled_y, scaled_x]
        cr, cg, cb = self.centers[cluster_id]
